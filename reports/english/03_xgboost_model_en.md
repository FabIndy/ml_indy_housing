...
These files already contain scaled data from the previous step (the scaler files are not reused here).
...
No additional transformations were applied since the standard and min-max scalers had already been used.
...
This transformation reduces skewness, stabilizes variance, and limits the impact of extreme values.
...
The XGBoost model with Pseudohuber loss and fine weighting achieves an excellent balance between robustness and accuracy:

It predicts standard homes (about 98% of the dataset) with an average error below 13%.

Luxury homes, rarer and more variable, show larger deviations but remain stably modeled.

This model constitutes the final reference for the ML Indy Housing pipeline, combining statistical rigor, adaptive weighting, and robustness to outliers.