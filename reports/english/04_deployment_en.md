...
1. **`indy-house-model/`**  
   ...
   This container serves as the **model repository** (endpoint for the backend).  
   > No `Dockerfile`: the Hugging Face container relies on a preconfigured image without an explicit Docker build.

2. **`house-price-api/`**  
   A **Dockerized** container based on **FastAPI**.  
   It was also uploaded and saved to the Hugging Face Hub.  
...
3. **`house-price-ui/`**  
   Container dedicated to the **Gradio user interface**, serving as a lightweight front-end for testing predictions.  
   It was uploaded and saved to the Hugging Face Hub.  
...
1. The user enters property characteristics into the **Gradio interface**.  
2. Gradio sends the request (JSON) to the **FastAPI backend** over HTTP.  
3. The backend queries the **model container** hosted on Hugging Face to obtain the prediction.  
4. The result (estimated price in USD) is returned to Gradio and displayed to the user.
